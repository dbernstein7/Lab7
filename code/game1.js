...
var actorChars = {
  ...
  "=": Lava, "|": Lava, "v": Lava, //"=" -> side scrolling lava, "|" -> vertical scrolling lava, "v" -> drip lava
  "R": SafeLava, "T": SafeLava, "G": SafeLava, //same as above
  "%": Potion,
  "^": AntiPotion,
  "y": Text
};
...
Potion.prototype.act = function(step) {
	this.wobble += step * textSpeed;
	var wobblePos = Math.cos(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

function AntiPotion(pos) {
	this.basePos = this.pos = pos.plus(new Vector(0.4,0.1));
	this.size = new Vector(0.4, 0.7);
	this.wobble = Math.random() * Math.PI *20;
}

AntiPotion.prototype.type = 'antiPotion';

AntiPotion.prototype.act = function(step) {
	this.wobble += step * textSpeed;
	var wobblePos = Math.cos(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Text.prototype.act = function(step) {
	this.wobble += step * textSpeed;
	var wobblePos = Math.cos(this.wobble) * textDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
function SafeLava(pos, ch) {
  ...
  if (ch == "R") {
    ...
    // Drip safeLava. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
...
var wobbleSpeed = 4, wobbleDist = 0.07;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" && this.status != "lost") {
    this.status = "lost";
    this.finishDelay = 1;
	jumpSpeed = 14;
  }
  else if (type == "safeLava" && this.status != "lost"){
	  if (this.status == "antiPotion") {
		  return;
	  }
	  else {
		  this.status = "lost";
  ...
  else if(type == "potion" && this.status != "lost") {
		this.actors = this.actors.filter(function(other) {
			return other != actor;
		});
		jumpSpeed = 22;
  ...
  else if(type == "antiPotion" && this.status != "lost") {
		this.actors = this.actors.filter(function(other) {
			return other != actor;
		});
		jumpSpeed = 14;
		this.status = "antiPotion";
  }
};
